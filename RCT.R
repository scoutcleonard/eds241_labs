...
bigN=20000 #number of observations we will draw

W <- runif(bigN,0,5) #uniform random variable - runif is distribution where chance of drawing anything between range is equal
X=as.integer(W+1)
X1 <- as.numeric(X==1)
X2 <- as.numeric(X==2)
X3 <- as.numeric(X==3)
...
#creating a covarience matrix w mean 0 
...
# we can control the average treatment effect here 
Y0 <- MU0 + e0
mean(Y0)
Y1 <- MU1 + e1
mean(Y1)
...
PO_DF <- data.frame(Y0,Y1,X) #po_df = potential outcome dataframe 
...
ggplot(PO_DF, aes(x=X, y=Y0)) +
  ...
  geom_point(position = jitter, color = "red", shape=1, aes(x=X, y=Y1)) #jitter symbols around the value by shifting the x values by small amounts, allowing us to visualize 
#plotting the relationship between potential outcomes and covariance x

#if ur conducting an experiment, your subjects enter now with all their random situations

# RANDOMLY ASSIGN A TREATMENT INDICATOR
D <- as.numeric((runif(bigN,0,1)) > 0.5)
mean(D) #roughly .5 as we expected 
...
RCT_DATA <- data.frame(D, Y0, Y1, X, X1, X2, X3, X4, X5)
